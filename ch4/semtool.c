...
#define SEM_RESOURCE_MAX 1 /* init value of all sema */
...
unsigned short get_member_count(int sid); /* positive or zero */
...
void opensem(int *sid, key_t key)
{
  ...
  if((*sid=semget(key,0,0666))==-1)
    {
      printf("Semaphore set doesn't exist!\n");
      exit(1);
    }
}
...
{
  int cntr;
  union semun semopts; /* val, semid_ds buf, short array */
  
  if(members > SEMMSL) {
    printf("Sorry, max number of semaphores in set is %d\n",SEMMSL);
    exit(1);
  }
...
void unlocksem(int sid, int member)
{
  struct sembuf sem_unlock={member,1,IPC_NOWAIT};/*release*/
  int semval;
  ...
  if((semop(sid,&sem_unlock,1))==-1)
    ...
    printf("Semaphore respurces incremented by one (unlocked)\n");
